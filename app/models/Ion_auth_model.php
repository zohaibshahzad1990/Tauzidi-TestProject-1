...
class Ion_auth_model extends MY_Model{
	...
	*/
	function install()
	{
		...
		$this->db->query("
				CREATE TABLE IF NOT EXISTS `users` (
				  ...
				  `forgotten_password_code` varchar(200),
				  `forgotten_password_time` varchar(200),
				  `remember_code` varchar(200),
				  `created_on` varchar(200),
				  ...
				  `ussd_pin` varchar(200),
				  `confirmation_code` varchar(200),
				  `created_by` varchar(200),
				  `modified_by` varchar(200),
				  `modified_on` varchar(200),
				  `expiry_time` varchar(200),
				  `user_account_activation_code` varchar(200),
				  `user_account_active` varchar(200),
				  `avatar` varchar(200),
				  PRIMARY KEY (`id`)
				) ENGINE=InnoDB DEFAULT CHARSET=utf8;
			");
		...
		if(!$this->db->count_all('users'))
		{
			...
			$this->register('0703656970','0703656970','aggreykoros04@gmail.com', $additional_data,array(1));
		}
	 ...
	 *
	 * Salt generation code taken from https://github.com/ircmaxell/password_compat/varchar(200)/master/lib/password.php
	 *
	 * @return void
	...
	public function identity_check($identity = '')
    {
        ...
        $this->db->where($this->dx($this->identity_column)." ='".$identity."'",NULL,FALSE);
    ...
    public function forgotten_password($identity,$remember_code = '')
    {
        ...
        $this->forgotten_password_code = generate_forgot_password_code(); //$key;
        ...
        $update = array(
            'forgotten_password_code' => valid_email($identity)?generate_forgot_password_code():$code,
	...
	public function google_signup($identity,$password,$email,$additional_data = array(),$groups = array(),$ignore_phone_validation=FALSE){
		if(valid_phone($identity) && $ignore_phone_validation == FALSE)
		{
			$identity = valid_phone($identity);
		}
	...
	public function google_signin($email = "",$id_token = ""){
        $this->trigger_events('pre_login');
        ...
        $this->trigger_events('extra_where');
        $this->select_all_secure($this->tables['users']);
        $this->db->where($this->dx($this->identity_column) . " = '" . $this->db->escape_str($email) . "'", NULL, FALSE);
        $this->db->limit(1);
        //die('mzee');
        $query = $this->db->get($this->tables['users']);
        if($query->row() && $query->row()->active !=1){
        ...
        if($query->num_rows() === 1){
            ...
            $this->update_id_token($user->id,$id_token);
            $this->update_last_login($user->id);
	...
	**/
	...
	public function update_last_login($id){
		...
		$this->update_secure_data($id,$this->tables['users'],$input);
		return $this->db->affected_rows() == 1;
	}

	public function update_id_token($id = 0,$id_token = ""){
		$input = array(
			'id_token' => $id_token,
			'modified_on' => time(),
		);
	...
	public function update_session_token($id){
		$this->trigger_events('update_last_login');
		...
		$input = array(
			'session_token' => generate_random_string(35),
