...
class Countries_m extends MY_Model {
	...
	function seed_countries(){
		...
		$countries_json = json_decode($countries);
		$country_deatils = $countries_json->countries->country;
		foreach ($country_deatils as $key => $country) {
			$input = array(
				'name'=>$country->countryName,
				//'slug'=>generate_slug($country->countryName),
				'active'=>1,
	...
	function get_country_by_calling_code($calling_code = ''){
		$this->select_all_secure('countries');
		$this->db->where($this->dx('calling_code'),$calling_code);
		return $this->db->get('countries')->row();
	}
	public function get_all(){	
		$this->select_all_secure('countries');
		$this->db->select(array($this->dxa('currency_code'),$this->dx('currencies.name').' as currency',$this->dxa('currency_code')));
		$this->db->join('currencies',$this->dx('currencies.country_id').' = countries.id','left');
		$this->db->order_by($this->dx('countries.name'), 'ASC', FALSE);
	...
	public function get_countries_and_currencies($id=0){
		$this->db->select( array(
			'countries.id as id',
			$this->dx('countries.name').' as name ',
			$this->dx('countries.active').' as active ',
			$this->dx('country_code').' as country_code ',
			$this->dx('countries.calling_code').' as calling_code ',
			$this->dx('currencies.name').' as currency ',
			$this->dx('currencies.currency_code').' as currency_code ') );
		$this->db->order_by($this->dx('countries.name'), 'ASC', FALSE);
		$this->db->where('countries.id',$id);
		$this->db->join('currencies','countries.id = '.$this->dx('country_id'),'LEFT');
		return $this->db->get('countries')->row();
	}
	...
	public function get_currency_options($show_currency_code = ''){
		$arr = array();
		$this->db->select(array('id',$this->dx('name').' as name',$this->dx('currency_code').' as currency_code'));
		$currencies = $this->db->get('countries')->result();
		foreach($currencies as $currency){
			if($show_currency_code=TRUE){
				$arr[$currency->id] = $currency->name.' ('.$currency->currency_code.')';
			}else{
	...
	public function get_currency_array($show_currency_code = ''){
		$arr = array();
		$this->db->select(array('id',$this->dx('name').' as name',$this->dx('currency_code').' as currency_code'));
		$currencies = $this->db->get('countries')->result();
		foreach($currencies as $currency){
			$arr[$currency->id] = $currency;
		}
		return $arr;
	}
}