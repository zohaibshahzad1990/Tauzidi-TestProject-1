...
class manager_menus_m extends MY_Model{

	protected $_table = 'manager_menus';
	...
	public function install(){
		$this->db->query("
		create table if not exists manager_menus(
	...
	public function get_parent_id_as_key_url_as_value_multidimensional_array(){
		...
		$manager_menus = $this->db->get('manager_menus')->result();
	...
	public function get_parent_id_as_key_menu_as_value_array(){
		$arr = array();
		$this->select_all_secure('manager_menus');
	...
	public function get($id = 0){
		$this->select_all_secure('manager_menus');
		$this->db->where('id',$id);
		return $this->db->get('manager_menus')->row();
	}

	function insert($input = array(),$skip_value = FALSE){
		return $this->insert_secure_data('manager_menus',$input);
	}
	...
	function generate_page_title($url=''){
		...
		$menu = $this->db->get('manager_menus')->row();
		if($menu){
	...
	function update($id,$input,$val=FALSE){
    	return $this->update_secure_data($id,'manager_menus',$input);
    }

	function get_all(){
		$this->select_all_secure('manager_menus');
		return $this->db->get('manager_menus')->result();
	}

	function count_all_active(){
		return $this->db->count_all_results('manager_menus');
	}
	...
	function get_active_links_in_array($manager_menus_array=array()){
		$menu_id_list = '0';
		$count = 1;
		foreach($manager_menus_array as $menu_id){
			if($menu_id){
				if($count==1){
	...
	function has_active_grand_children($parent_id=0){
		...
		$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
		return $this->db->count_all_results('manager_menus')>0?TRUE:FALSE;
	}
		...
		if(!empty($links)){
			...
			foreach($links as $link):
				echo '<li class="dd-item" data-id="'.$link->id.'">
	                        ...
	                        echo '</div>';
	                        $this->manager_menus_m->display_children($link->id);
	            echo '</li>';
	        endforeach;
			echo '</ol>';
		}
	...
	function generate_header_manager_menus(){
...
function generate_side_bar_sub_menu($parent_id = 0,$acceptable_children=array(),$check_children=FALSE,$parent_name = ""){
  ...
  if($children_links){
	...
	function get_menu_by_link_url($link_url=''){
		...
		$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
		$this->db->order_by($this->dx('position').'+0','ASC', FALSE);
		return $menu = $this->db->get('manager_menus')->row();
	}
	...
	function generate_page_quick_action_manager_menus(){
		$link_url = uri_string();
