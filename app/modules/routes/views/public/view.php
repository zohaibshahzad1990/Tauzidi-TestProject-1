<div>
		...
		<div class="m-portlet__head m-portlet__head--fit">
			<div class="m-portlet__head-caption">
				<div class="m-portlet__head-action">
					<button type="button" class="btn btn-sm m-btn--pill m-btn--air btn-brand mr-2">Profile</button>
					...
					<a href="<?php echo site_url("users/users/recommendations/".$user->id); ?>" class="btn btn-sm m-btn--pill m-btn--air btn-info"><?php echo $recommendations_count; ?> Recommendations</a>
				</div>
			</div>
		</div>
	...
	<h3 class="fancy_title my-3">
		<?php echo $user->first_name." ".$user->last_name; ?> Posts
	</h3>
...
<script>
	$(document).ready(function(){
		...
		window.selected_tab = 'discover';
  		$(document).on('click','#buying',function(e){
			window.selected_tab = 'buying';
  			//alert("Am in");
  			$(this).addClass('active');
  			$(this).addClass('show');
  			$('#discover,#selling').removeClass('active').removeClass('show');
  			$('.selling-post').hide();
  			$('.buying-post').show();
  			e.preventDefault();
  		});

  		$(document).on('click','#selling',function(e){
			window.selected_tab = 'selling';

  			$(this).addClass('active');
  			$(this).addClass('show');
  			$('#discover,#buying').removeClass('active').removeClass('show');
  			$('.selling-post').show();
  			$('.buying-post').hide();
  			e.preventDefault();
  		});
  		...
  		$(document).on('click','#discover',function(e){
			window.selected_tab = 'discover';
  			...
  			$('#selling,#buying').removeClass('active').removeClass('show');
  			$(this).addClass('active');
  			$(this).addClass('show');
  			$('.selling-post').show();
  			$('.buying-post').show();
	...
	$(window).on('load',function(){
    	...
    	load_users_posts(user_id);
	});

	$(window).scroll(function() {
	    var top_of_element = $("#load_more_posts").offset().top;
	    var bottom_of_element = $("#load_more_posts").offset().top + $("#load_more_posts").outerHeight();
	    var bottom_of_screen = $(window).scrollTop() + $(window).innerHeight();
	    var top_of_screen = $(window).scrollTop();

	    if ((bottom_of_screen > top_of_element) && (top_of_screen < bottom_of_element)){
	        //load more posts
	        if(window.selected_tab == 'buying'){
	        	var start = $('.buying-post').length;
	        }else if(window.selected_tab == 'selling'){
	        	var start = $('.selling-post').length;
	        }else{
	        	var start = $('.cust_card').length;
	        }
	        var limit = 10;
	        $('#load_more_posts').remove();
	        //alert(window.selected_tab);
	        lazy_load_users_posts(user_id,start,limit,window.selected_tab);
