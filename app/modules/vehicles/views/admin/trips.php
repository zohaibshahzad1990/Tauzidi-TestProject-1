<div class="kt-portlet" id="portlet_body">
    <div class="kt-portlet__head">
        ...
        <div class="kt-portlet__head-toolbar">
            ...
            <button id="add_drop_points" data-title="Assign Trip" data-pay-bill-id="" data-content="#assign-trip-form-holder" class="btn btn-default launch-modal-lg">
				<i class="fa fa-car"></i> Assign Trips
			</button>
		</div>
    </div>
...
<div class='d-none' id="assign-trip-form-holder">
    <div id="start-trip-form">
        ...
        <div id="" class="kt-portlet__body">
        	<?php echo form_hidden('vehicle_id',$vehicle->id);?>
        	<input type="hidden" name="vehicle_assigned_id" id="vehicle_assigned_id" />
			<?php echo form_dropdown('trip_id',array(''=>'Select Trip')+$trips,'','class="form-control modal_select2" id="trip_id"');?>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
	   ...
	   /*$(window).on('shown.bs.modal', function() {
	        $(".modal .kt-select2-modal").select2({
	            placeholder:{
	                id: '-1',
	                text: "--Select option--",
	            }, 
	...
	$("#add_drop_points").on('click', function(){
		 $("#start-trip-form #vehicle_assigned_id").val("");
	})
	...
	$(document).on('submit','#modal-form',function(e){
        ...
        if(form.find('#start-trip-form').is(':visible')){
            var pay_bill_id = $('input[name=id]').val();
                ...
                $.ajax({
                    type: "POST",
                    url: base_url+'/ajax/vehicles/assisgn_trip',
                    data: form.serialize(),
                    dataType: "json",
    ...
    $(document).on('click','.prompt_confirmation_message_link',function(){
        ...
        swal.fire({
            ...
            if(e.value == true){
                ...
                $.ajax({
                    type:'POST',
                    url:base_url+'/ajax/vehicles/delete_assisgned_trip',
                    data:{'id':id},
                    dataType: "json",
                    success: function(response){
                        if(isJson(response)){
    ...
    $('body').on('click','#edit_point',function(){
	    ...
	    $.ajax({
            url : base_url+"/ajax/vehicles/get_assigned_trip",
            method : "POST",
            data:{ id: id},
            success: function (response) {
                ...
                if(result.result_code == 200){
                    ...
                    var id = data.trip_id
                    console.log(data.id)
			        $("#start-trip-form #trip_id").val(id).trigger("change");
                    $("#start-trip-form #vehicle_assigned_id").val(data.id);
					KTApp.unblock('#drop_point_list');
                }else{
	...
	function load_vehicle_trips(){
		var id = "<?php echo $vehicle->id;?>"
		KTApp.block('#drop_point_list');
	    var load_drop_points = $.ajax({
            type: "GET",
            url:base_url+'/ajax/vehicles/get_vehicle_trips/'+id,
            dataType : "html",
                success: function(response) {
