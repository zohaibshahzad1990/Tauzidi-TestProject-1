...
class Ion_auth{
			...
			if ($user)
			{
				...
				if(!$this->config->item('use_ci_email', 'ion_auth'))
				{
					...
					if(valid_email($identity))
					{
						...
						$message = $this->emails_m->build_email_message('forgot-password',array(
							'FIRST_NAME'=>$user->first_name,
							'LAST_NAME' => $user->last_name,
							'LINK' => site_url('/auth/reset-password/'.$user->forgotten_password_code),
							'APPLICATION_NAME' => $this->settings->application_name,
							'YEAR'		=>	date('Y',time()),
							'DATE' 		=>	date('d-m-Y',time()),
						));
						$subject = $this->settings->application_name.' Reset Password';
						$success = $this->messaging_manager->send_password_recovery_email($identity,$subject,$message);
					}
					else if(valid_phone($identity))
					{
						...
						//$success = $this->messaging_manager->send_password_recovery_sms(valid_phone($identity),$message,$user->id);
						$success = $this->sms_m->send_system_sms(valid_phone($identity),$message,$user->id); 
						$success = true;
						$this->session->set_flashdata('success'," Password Recovery Code sent to ".$identity);
					}
	...
	public function get_user_group_details($id = 0){
		$arr = array();
		if($id){
			$user_groups = $this->ion_auth->get_users_groups($id)->result();
			foreach($user_groups as $user_group){
	...
	public function get_user_group_option_details($id = 0){
		$arr = array();
		if($id){
			$user_groups = $this->ion_auth->get_users_groups($id)->result();
			foreach($user_groups as $user_group){
				$arr[generate_slug($user_group->name)] = $user_groups;
			}
		}
		return $arr;
	}
